...
class Channel(core.base):
    ...
    def loadValues(self):
        ...
        self.HEADAMP_SOURCE, = self.OSC.send(
        ...
        self.HP_FREQ, = self.OSC.send(osc.construct(
            f'/{self.TYPE}/{str(self.ID).zfill(2)}/preamp/hpf'))
    ...
    def loadAllBusSends(self) -> List[BusSends]:
        ...
        for i in range(16):
            on, = self.OSC.send(osc.construct(
    ...
    def updateValuesInDb(self, db: database.controller, saveId: int, channelId: int):
        db.execute(
            f'UPDATE channels SET headamp_source = "{self.HEADAMP_SOURCE}", headamp_gain = "{self.HEADAMP_GAIN}", hp_on = "{self.HP_ON}", hp_freq = "{self.HP_FREQ}", phantom = "{self.PHANTOM}" WHERE save_id = {saveId} AND channel_num = {channelId}')
        baseId = db.execute(
            f'SELECT base_id from channels WHERE save_id = {saveId} AND channel_num = {channelId}')
        if baseId:
    ...
    def loadValuesFromDb(self, db: database.controller, saveId: int, channelId: int):
        vals = db.execute(
            f'SELECT base_id, headamp_gain, hp_on, hp_freq, phantom from channels WHERE save_id = {saveId} AND channel_num = {channelId}')
    ...
    def updateBusSendMutes(self, busNum: int, newVal: bool):
        ...
        self.OSC.send(osc.construct(
            f'/{self.TYPE}/{str(self.ID).zfill(2)}/mix/{str(busNum + 1).zfill(2)}/on', [{'f': newVal}]))
    ...
    def updateBusSendLevels(self, busNum: int, newVal: float):
        if newVal >= 0 and newVal <= 1:
            ...
            self.OSC.send(osc.construct(
                f'/{self.TYPE}/{str(self.ID).zfill(2)}/mix/{str(busNum + 1).zfill(2)}/level', [{'f': newVal}]))
    ...
    def updateHeadampGain(self, val: float) -> None:
        ...
        if val >= -12 and val <= 60:
            ...
            self.OSC.send(osc.construct(
                f'/headamp/{str(self.HEADAMP_SOURCE).zfill(3)}/gain', [{'f': self.HEADAMP_GAIN}]))
        else:
            self.triggerError(
                'Headamp Gain is not a valid float between -12 and 60')

    def updateHighPassToggle(self, val: bool) -> None:
        val = bool(val)
        self.HP_ON = val
        self.OSC.send(osc.construct(
            f'/{self.TYPE}/{str(self.ID).zfill(2)}/preamp/hpon', [{'i': 1 if self.HP_ON else 0}]))
    ...
    def updateHighPassFreq(self, val: float) -> None:
        ...
        if val >= 0 and val <= 400:
            self.HP_FREQ = val
            self.OSC.send(osc.construct(
                f'/{self.TYPE}/{str(self.ID).zfill(2)}/preamp/hpf', [{'f': self.HP_FREQ}]))
        else:
            self.triggerError(
                'Highpass frequency is not a valid float between 20 and 400')

    def updatePhantomPowerToggle(self, val: bool) -> None:
        val = bool(val)
        self.PHANTOM = val
        self.OSC.send(osc.construct(
            f'/headamp/{str(self.HEADAMP_SOURCE).zfill(3)}/phantom', [{'i': 1 if self.PHANTOM else 0}]))
